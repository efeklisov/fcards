...
fastify.get("/translate", async (req, reply) => {
  if (req.query.word == "") return reply.sendFile("empty_word.html");

  try {
    STATE.translatedWord = await db.getTranslation(req.query.word);

    if (STATE.translatedWord.word != "")
      return reply.view("word.ejs", {
        json: [STATE.translatedWord],
        shorten: 0,
        buttons: 0,
        gameButtons: 0,
        offset: 0,
...
fastify.get("/save", async (req, reply) => {
  if (STATE.translatedWord.word == "") return reply.sendFile("empty.html");

  try {
    STATE.wordCount = await db.insertAndGetCount(
      STATE.translatedWord.word,
...
fastify.get("/render_vault", async (req, reply) => {
  try {
    ...
    return reply.view("word.ejs", {
      json: (await db.getRows(len, idx)).map(JSON.parse).reverse(),
      shorten: 0,
      buttons: 1,
      gameButtons: 0,
      offset: 0,
...
const shipPages = (reply) => {
  const maxPages = Math.ceil(STATE.wordCount / WORDS_PER_PAGE);
  const currPage =
    Math.floor(STATE.watchIndex / WORDS_PER_PAGE) + (maxPages ? 1 : 0);

  return reply.view("pages.ejs", {
    currPage: currPage,
    maxPages: maxPages,
...
fastify.get("/public/vault.html", async (req, reply) => {
  STATE.watchIndex = 0;
  return shipPages(reply);
});

fastify.get("/next_words", async (req, reply) => {
  if (req.query.reset) STATE.watchIndex = 0;
  else if (STATE.watchIndex + WORDS_PER_PAGE < STATE.wordCount)
...
fastify.get("/prev_words", async (req, reply) => {
  if (req.query.reset) STATE.watchIndex = 0;
  else STATE.watchIndex = Math.max(STATE.watchIndex - WORDS_PER_PAGE, 0);
...
fastify.get("/full_next_words", async (req, reply) => {
  if (req.query.reset) STATE.watchIndex = 0;
    ...
    STATE.watchIndex =
      Math.floor(STATE.wordCount / WORDS_PER_PAGE) * WORDS_PER_PAGE;
...
fastify.get("/random_word", async (req, reply) => {
  if (!STATE.wordCount) return reply.sendFile("game_empty.html");

  const randNum = Math.floor(
    Math.random() * (STATE.wordCount + STATE.allMisses),
  );
  let translation;
  try {
...
fastify.get("/show_result", async (req, reply) => {
  ...
  return reply.view("game_results.ejs", {
