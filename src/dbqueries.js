...
const getRows = async (n, m) => {
  return new Promise((resolve, reject) => {
    ...
    db.serialize(() => {
      const sql =
        "SELECT rowid AS id, word, translation FROM Words LIMIT ? OFFSET ?";
      db.each(
        sql,
        [n, m],
        function (err, row) {
...
const getResults = async (n, g) => {
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      db.get(
        "SELECT rowid as id, * FROM Words WHERE rowid = ?",
        [n],
        (err, row) => {
          if (err) {
            reject(err);
            ...
            ret = {
              guesses: row.guesses + (g ? 1 : 0),
              misses: row.misses + (g ? 0 : 1),
            };
            resolve(ret);

            db.run(
              "UPDATE Words SET guesses = ?, misses = ? WHERE rowid = ?",
              ret.guesses,
              ret.misses,
              row.id,
            );
